...
public class LiJiaCunShiChang extends Base{
	int mapData[][] ;
	...
	int lxyX = 100 ;
	int lxyY = 700 ;
	Rectangle rect = new Rectangle( 0 , 742 , 65 , 165 ) ;
	/**	`
	 * 初始化场景数据
	 */
	public LiJiaCunShiChang(){
		try{
			mapImage = ImageIO.read( new File("E:/ftp/ljc/0.png") ) ;
			mapdata=ImageIO.read(new File("e:/ftp/ljcdata.png"));
			zgwImage=ImageIO.read(new File("e:/ftp/ljc/zgw.png"));
			for( int i = 0 ; i < 8 ; i ++ ){
